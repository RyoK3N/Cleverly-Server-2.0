...
─────────────────────
1. Fetch Monday.com board data and Calendly scheduled events concurrently,
   then annotate Monday data with origin based on Calendly invitee e-mails.

2. Persist the combined data to a timestamped CSV, split each Monday "Group"
   into its own CSV inside a fresh `sessions/` folder, and also write a
   Group-keyed JSON file for downstream dashboards.

3. Show rich progress bars (tqdm) and structured logs at every step.
"""
...
# ─── LOGGING SET-UP ───────────────────────────────────────────────────────────
dotenv.load_dotenv()  # pick up env first, incl. LOG_LEVEL
...
#Import your helpers — adjust path as needed.
try:
...
class MondayDataProcessor:
    ...
    def fetch(self) -> pd.DataFrame:
        ...
        logger.debug("Fetched %d groups from board %s", len(groups),
                     self.cfg.BOARD_ID)

        segments: list[pd.DataFrame] = []
        for gid, nice_name in tqdm(self.cfg.GROUP_MAPPING.items(),
                                   desc="Monday groups",
                                   unit="grp"):
...
def main(debug: bool = False) -> None:
    ...
    monday_df.loc[monday_df["Email"].isin(invitee_emails),
                  "origin"] = "cold-email"
